...
plt.bar([f"Nhà máy {i+1}" for i in range(N)], total_production_per_factory, color='skyblue')
plt.xlabel("Nhà máy")
...
for j in range(M):
    plt.bar(index + j * bar_width, X.value[:, j], bar_width, label=f"Sản phẩm {j+1}")

plt.xlabel("Nhà máy")
plt.ylabel("Số lượng sản phẩm sản xuất")
plt.title("Biểu đồ số lượng sản phẩm sản xuất tại mỗi nhà máy")
plt.xticks(index + bar_width * (M - 1) / 2, [f"Nhà máy {i+1}" for i in range(N)])
plt.legend()
plt.show()
